...
def _results_to_csv(results):
    ...
    for filename, result in results.iteritems():
        err_list = ','.join([x.message for x in result.errors
        ...
        if result.c2s_result.end_time and result.c2s_result.start_time:
            row['c2s_duration'] = (result.c2s_result.end_time -
                                   result.c2s_result.start_time).total_seconds()
        if result.s2c_result.end_time and result.s2c_result.start_time:
            row['s2c_duration'] = (result.s2c_result.end_time -
                                   result.s2c_result.start_time).total_seconds()
